...
- Offline banner: a slim bar appears below the top bar when the device has no network connection. Messages are still accepted while offline (queued as `pending`) and sent automatically when connectivity returns. ✓
